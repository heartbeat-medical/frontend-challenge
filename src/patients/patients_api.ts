...
import { isPatients, PatientsService, Patient, PatientSearchQuery } from "./patients";
...
function patientMatchesQuery(query: PatientSearchQuery, patient: Patient) : boolean {
    if (query.ehrID && patient.ehrID.toLowerCase().includes(query.ehrID.toLowerCase())) {
        return true
    }
